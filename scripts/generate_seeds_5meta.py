...
def main(args):
    ...
    for sub in subjects:
        ...
        # Get session name (if any)
        ses = imgs.parent.parent.name

        # Track which subclass folders are missing the subject
        subclasses_to_run = []

        for subclasses in range(1, max_subclusts):
            if ses == "":
                out_suffix = f"subclasses_{subclasses}/{sub.name}/anat/"
            else:
                out_suffix = f"subclasses_{subclasses}/{sub.name}/{ses}/anat/"
        ...
        # Skip subject only if it's already processed in all subclasses
        if not subclasses_to_run:
            print(f"Skipped {sub.name}")
            continue

        # Schedule processing only for missing subclasses

...
# INPUT PATH (BIDS)
bids_path = Path("/media/vzalevskyi/data/atlasses/CHN-fetal-brain-atlas/chn_bids")
...
resampe_transform = monai.transforms.Spacingd(
    keys=["image", "label"],
    pixdim=(res, res, res),
...
cropper = monai.transforms.CenterSpatialCropd(
    keys=["image", "label"], allow_missing_keys=True, roi_size=target_size
)
